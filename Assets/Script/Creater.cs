...
public class Creater : MonoBehaviour {
	...
	void create () {
		int hinder = Random.Range (0, roundBatch);
		int colorInd = Random.Range (0, colorList.Length);
		int mod = roundBatch / Random.Range (1, hinderMod + 1);
		...
		for (int i = 0; i < totalBatch; i++) {
			for (int j = 0; j < roundBatch; j++) {
				...
				if (!firstBatch && (j % mod == hinder % mod) && i >= paddingBatch && i < paddingBatch + lenBatch) {
					float rate = Mathf.Abs(i - centerBatch) / (float)centerBatch;
